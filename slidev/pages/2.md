## Dask Array Fundamentals
- Parallel NumPy for Big Data: Dask Array implements a subset of the NumPy ndarray interface using blocked algorithms – it cuts a large array into many small numpy-array chunks and coordinates their computation in parallel. This lets us work with arrays larger than memory by using all cores and streaming data from disk.
- Lazy Evaluation: Operations on Dask arrays are lazy. No actual computation or loading happens until you explicitly ask for a result. You can build up a pipeline of operations and Dask will execute it when you call .compute() (or convert to a NumPy array). Example: after creating a Dask array from files, “no data has been read from disk yet” ￼ – only when you compute or index it will data be loaded.
- Chunks and Tasks: A Dask array consists of many chunks (blocks), each a small NumPy array. Computations are broken into many tasks – one per chunk (plus some aggregation tasks). This parallelizes work across chunks and across CPU cores. Dask’s scheduler handles running these tasks asynchronously.
- Chunk Size Trade-offs: Choosing chunk sizes is critical for performance. If chunks are too small, the overhead of scheduling tasks (roughly ~1ms per task) can dominate runtime – tasks should ideally take >100ms each ￼. If chunks are too large, you might run out of memory and lose parallelism. In practice, chunks in the range of tens of MBs up to ~1GB are common ￼, depending on your RAM and the complexity of computations.

Jupyter representation of a Dask array: This example shows a 4D Dask array (~10.22GB total, uint16) composed of 1200 chunks of size ~8.5MB each (shape per chunk: 1×65×256×256). The table (left) details the array’s total bytes, shape, chunk count, etc., and the visual (right) depicts chunk divisions. Dask will schedule ~1200 tasks to process these chunks in parallel. Importantly, none of the chunks are loaded into memory until a computation is actually triggered (the array remains lazy).
